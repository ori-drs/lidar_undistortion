...
TEST(TestPoseBuffer, oneMillionRandomValues){
  ...
  for(int i = 0; i < 1000000; i++){
    ...
    EXPECT_TRUE(result.isApprox(expected, 1e-14))
         << "FAILURE!!" << std::endl
         << "alpha      : "<< std::setprecision(15) << alpha << std::endl
         << "after_alpha: "<< std::setprecision(15) << time_alpha << std::endl
         << "time1 = " << time1 << std::endl
         << "time_inter = " << time_inter << std::endl
         << "time2 = " << time2 << std::endl
         << std::endl << result.matrix() << std::endl
         << std::endl << expected.matrix() << std::endl
         << std::endl << expected.matrix() - result.matrix();
